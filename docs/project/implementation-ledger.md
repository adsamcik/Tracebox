...
| X3.6 | Crash dispatch/coexistence state machine | X3.3;X3.4;X3.5 | Exactly one primary/fallback result | IN_PROGRESS |  | Typed core now has a host-tested native dispatch plan encoding `tracebox_bridge.cc`'s preserved prior sigaction and single re-raise behavior. A true JNI/native policy handoff remains pending. |
...
| X3.8 | Rust panic/fault integration | C1.5;X3.6 | Contained panic and native fault paths | IN_PROGRESS |  | Bounded global hook and cfg-gated unwind containment are cargo-tested. The native structured-record bridge (`tb_tracebox_record_panic_v1`, a bounded `extern "C"` ABI struct with boundary panic containment) is now real and cargo-tested. A release (`panic="abort"`) probe binary was built and run, confirming abort-mode termination (process exit `-1073740791`, not a graceful return) rather than unwind; this is host/Windows-level Rust-panic-strategy evidence, not Android/ART-level validation. |
| X3.9 | Live ANR watchdog | F0.6;X3.3;R2.1;R2.5 | Adaptive bounded watchdog | IN_PROGRESS |  | The actual Android `AnrWatchdog` now consumes main-looper delay/stack timestamps through `AnrStateMachine` and requests a snapshot only after its credible-candidate transition; a host binding test exercises lifecycle and repeated heartbeats. Device measurement remains unperformed; responsive-looper limitation is documented in design section 13.6. |
| X3.10 | Exit reconciliation | C1.6;R2.4;R2.5;R2.8;X3.6;X3.7;X3.8;X3.9 | Policy-safe idempotent OS exit import | IN_PROGRESS |  | Host-tested synthetic `ApplicationExitInfo` logic derives installation-lifetime source keys, has a hard no-eviction tombstone cap that disables new import on exhaustion, and links `UNMATCHED`/`POSSIBLE`/`PROBABLE`/`EXACT`. The live Android API adapter and API 37 `AnrInfo` remain untested because API 37 emulator infrastructure is `UNAVAILABLE_EXTERNAL`. |
