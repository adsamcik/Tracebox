...
| F0.2 | Pin toolchains and dependencies | F0.1 | Gradle, AGP, Kotlin, JDK, NDK, CMake, Rust, Crashpad revisions locked and verified | PASS | 4e7959b70097426b253dda3671eef4a8b16ae1d9 | Crashpad reuse now rehashes the complete expected post-patch source tree against a tracked lock value, ignoring mutable manifest claims; the targeted tamper test reconstructs the immutable source and restores the expected hash. See `evidence/phase0/review-fix-host-validation.json`. |
...
| F0.5 | Emergency fallback spike | F0.2 | Fixed signal record survives pre-Durable and Crashpad-unavailable faults | FAIL |  | Targeted API 30 proof now covers a killed handler, exactly one valid fallback record, zero raw-dump delta, prior-action chaining exactly once, and observable signal death. The complete required matrix was not rerun and API 37 remains blocked. |
