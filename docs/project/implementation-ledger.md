...
| C1.4 | Versioned C ABI | C1.2 | Size-prefixed ABI and compatibility tests | IN_PROGRESS |  | Generated C event layouts and the emergency public view use v1 headers; the Phase 0 raw 256-byte storage remains internal. Current host lacks a C compiler, so the changed C ABI tests are not rerun. Evidence: `evidence/phase1/native-abi.json`. |
| C1.5 | Rust bindings/wrapper | C1.4 | Generated sys crate and safe wrapper | IN_PROGRESS |  | Exported size-prefixed `extern "C"` functions catch unwinds and Rust tests pass, but its C ABI dependency awaits host C validation. Evidence: `evidence/phase1/rust-bindings-and-identities.json`. |
