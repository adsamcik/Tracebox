...
| C1.6 | Identity and build contract | F0.2;C1.2 | Internal identities and build/symbol catalog | PASS |  | Opaque typed identity allocation journals before return; summaries are deterministic and journaled. Applying the Gradle plugin writes a schema/project provenance catalog. R8/ELF symbol capture remains Phase 5. Evidence: `evidence/phase1/kotlin-api-and-identity.json`; `evidence/phase1/rust-bindings-and-identities.json`. |
